...
export default function BarItem({name, number, stage, setStage}) {
    ...
    return (
        <div className={`w-100 text-center d-flex flex-column gap-3`}>
            <h2 className={`${highlight ? styles.completed_green_h2 : ''} ${styles.h2} ${(stage > number) ? styles.cursor : ''}`}
                onClick={() => {if (stage > number) {setStage(number)}}}
            >{name}</h2>
            <div className={`d-flex align-items-center `}>
                <div className={`${styles.progress_line}  ${highlight ? styles.completed_green_line : ''}`}></div>
                <div className={`${styles.progress_number_wrapper}  ${highlight ? styles.completed_green_number_wrapper : ''} ${(stage > number) ? styles.cursor : ''}`} 
                    onClick={() => {if (stage > number) {setStage(number)}}}>
